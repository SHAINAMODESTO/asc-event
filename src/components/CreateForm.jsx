...
// - candidate attendee fields that tell us it's already been distributed
// - the service call that marks it distributed
//
// The backend rejects a second PATCH with a 400 ("Attendee has already
// received the door prize"), which means it IS tracking this somewhere
// on the attendee record — we just don't know the exact field name yet.
// `distributedFields` lists every reasonable guess; the first truthy one
// wins. Once you confirm the real field (check the Network tab response
// for GET /attendee/:id, or the "FULL RESPONSE" console.log already in
// handleViewAttendee below), trim this down to just that field name.
...
const GIVEAWAY_TYPES = [
  {
    ...
    distributedFields: [
      "lootBagDistributedAt",
      "lootBagDistributed",
      "lootBagReceived",
      "lootBagReceivedAt",
      "receivedLootBag",
      "isLootBagDistributed",
    ],
  ...
  },
  {
    ...
    distributedFields: [
      "souvenirDistributedAt",
      "souvenirDistributed",
      "souvenirReceived",
      "souvenirReceivedAt",
      "receivedSouvenir",
      "isSouvenirDistributed",
    ],
  ...
  },
  {
    ...
    distributedFields: [
      "doorPrizeDistributedAt",
      "doorPrizeDistributed",
      "doorPrizeReceived",
      "doorPrizeReceivedAt",
      "receivedDoorPrize",
      "isDoorPrizeDistributed",
    ],
    action: distributeDoorPrize,
  },
];

// Returns the first truthy distribution value found on the attendee
// record for this giveaway (a timestamp string, or `true`), or
// undefined if none of the candidate fields are set.
...
// ========================================
// The local safety-net state further down (`locallyDistributed`) used
// to live only in this component's React state, which meant marking a
// giveaway here and then switching to the QR Scanner page (a separate
// route/component) forgot it — the scanner had no way to know it was
// already distributed unless the real backend field name happened to
// match one of our guesses above.
//
// Persisting the same cache to localStorage (keyed per event) fixes
// that: whichever screen marks a giveaway writes it here, and both
// EventAttendees and QRScanner read it back on load, so "already
// distributed" stays true everywhere in this browser — the same way
// check-in status already stays true everywhere, because check-in
// relies on the real `status` field instead of a guess.
//
// Caveat: this is per-browser, not per-account, so it won't sync
// across two different devices/coordinators the way a real backend
// field would. Once the true field name is confirmed, `distributedFields`
// above can be trimmed to just that field and this cache becomes a
// pure bonus rather than the primary source of truth.
...
// Local safety net: once we've confirmed (via a successful PATCH, or
// via the backend telling us it was already done) that a giveaway is
// distributed for an attendee, remember it here even if none of our
// `distributedFields` guesses match what the API actually returns.
...
// refresh or the correct field name.
...
const handleDistributeGiveaway = async (giveaway) => {
  ...
  try {
    ...
    // The backend is already tracking this attendee/giveaway pair —
    // it just rejected our PATCH because it was already marked. Treat
    // that as "already distributed" rather than a hard failure, since
    // none of our guessed field names may match what it returns.
    if (
